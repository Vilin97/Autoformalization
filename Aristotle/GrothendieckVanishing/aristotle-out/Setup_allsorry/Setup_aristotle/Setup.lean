...
/-! ### Auxiliary definitions (originally in separate module) -/

/-- The sheaf value at ⊥ is terminal. -/
noncomputable def isTerminal_sheaf_bot {X : TopCat.{u}}
    (F : TopCat.Sheaf AddCommGrpCat.{u} X) :
    IsTerminal (F.val.obj (op (⊥ : Opens X))) :=
  F.isTerminalOfEqEmpty rfl

/-- `ULift ℤ` is projective in `AddCommGrpCat`. -/
theorem ulift_int_projective : Projective (AddCommGrpCat.of (ULift.{u} ℤ)) := by
  constructor
  intro E X f e he
  rw [AddCommGrpCat.epi_iff_surjective] at he
  obtain ⟨y, hy⟩ := he (ConcreteCategory.hom f ⟨1⟩)
  refine ⟨AddCommGrpCat.ofHom
    (AddMonoidHom.mk' (fun z => z.down • y) (by intro a b; simp [add_smul])), ?_⟩
  ext ⟨n⟩
  simp only [AddCommGrpCat.hom_comp, AddCommGrpCat.ofHom, AddMonoidHom.coe_comp,
    Function.comp_apply]
  show ConcreteCategory.hom e (n • y) = ConcreteCategory.hom f ⟨n⟩
  rw [map_zsmul, hy, ← map_zsmul]
  congr 1; ext; simp

/-- Natural isomorphism `Γ ≅ sheafSections(⊤)` via uniqueness of right adjoints. -/
noncomputable def Sheaf.ΓNatIsoSheafSections {X : TopCat.{u}}
    (J : GrothendieckTopology (Opens X))
    (A : Type*) [Category A] [HasWeakSheafify J A]
    [HasGlobalSectionsFunctor J A]
    (hTop : IsTerminal (⊤ : Opens X)) :
    Sheaf.Γ J A ≅ (sheafSections J A).obj (op ⊤) :=
  (constantSheafΓAdj J A).rightAdjointUniq (constantSheafAdj J A hTop)

set_option synthInstance.maxHeartbeats 320000 in
set_option maxHeartbeats 800000 in
/-- Sheaf cohomology `H^n(X, F) := Ext^n(ℤ_X, F)`. -/
noncomputable def Sheaf.H {X : TopCat.{u}}
    (F : TopCat.Sheaf AddCommGrpCat.{u} X) (n : ℕ) : Type u :=
  Abelian.Ext ((constantSheaf (Opens.grothendieckTopology X) AddCommGrpCat.{u}).obj
    (AddCommGrpCat.of (ULift.{u} ℤ))) F n

set_option maxHeartbeats 400000 in
/-- If Ext^n of both ends of a SES vanish, so does the middle. -/
theorem subsingleton_ext_of_ses_middle {C : Type*} [Category C]
    [Abelian C] [HasExt C] {Z : C}
    {S : ShortComplex C} (hS : S.ShortExact) (n : ℕ)
    (h₁ : Subsingleton (Abelian.Ext Z S.X₁ n))
    (h₃ : Subsingleton (Abelian.Ext Z S.X₃ n)) :
    Subsingleton (Abelian.Ext Z S.X₂ n) := by
  ...
  obtain ⟨a', ha'⟩ := Ext.covariant_sequence_exact₂ Z hS a ha
  obtain ⟨b', hb'⟩ := Ext.covariant_sequence_exact₂ Z hS b hb
  rw [← ha', ← hb']; congr 1; exact @Subsingleton.elim _ h₁ _ _
...
-- Helper: naturality of NatTrans applied to concrete elements
private lemma naturality_concrete {X : TopCat.{u}}
    {F G : TopCat.Sheaf AddCommGrpCat.{u} X}
    (α : F ⟶ G) {U V : Opens X} (i : U ⟶ V)
    (x : F.val.obj (op V)) :
    ConcreteCategory.hom (G.val.map i.op)
      (ConcreteCategory.hom (α.val.app (op V)) x) =
    ConcreteCategory.hom (α.val.app (op U))
      (ConcreteCategory.hom (F.val.map i.op) x) := by
  have h := α.val.naturality i.op
  have : ConcreteCategory.hom (α.val.app (op V) ≫ G.val.map i.op) x =
         ConcreteCategory.hom (F.val.map i.op ≫ α.val.app (op U)) x := by
    rw [h]
...
-- Helper: composition of restriction maps
private lemma res_comp_eq {X : TopCat.{u}}
    {F : TopCat.Sheaf AddCommGrpCat.{u} X}
    {A B C : Opens X} (h₁ : A ≤ B) (h₂ : B ≤ C)
    (x : F.val.obj (op C)) :
    ConcreteCategory.hom (F.val.map (homOfLE h₁).op)
      (ConcreteCategory.hom (F.val.map (homOfLE h₂).op) x) =
    ConcreteCategory.hom (F.val.map (homOfLE (h₁.trans h₂)).op) x := by
  have key : F.val.map (homOfLE h₂).op ≫ F.val.map (homOfLE h₁).op =
      F.val.map (homOfLE (h₁.trans h₂)).op := by
    rw [← Functor.map_comp]; rfl
  have step1 : ConcreteCategory.hom (F.val.map (homOfLE h₁).op)
      (ConcreteCategory.hom (F.val.map (homOfLE h₂).op) x) =
      AddCommGrpCat.Hom.hom (F.val.map (homOfLE h₂).op ≫ F.val.map (homOfLE h₁).op) x := by
    simp [AddCommGrpCat.hom_comp]
  rw [step1, key]

-- Helper: restriction commutes with g application
private lemma res_g_eq {X : TopCat.{u}}
    {S : ShortComplex (TopCat.Sheaf AddCommGrpCat.{u} X)}
    {V W : Opens X} (h : W ≤ V)
    (t : S.X₂.val.obj (op V)) :
    ConcreteCategory.hom (S.g.val.app (op W))
      (ConcreteCategory.hom (S.X₂.val.map (homOfLE h).op) t) =
    ConcreteCategory.hom (S.X₃.val.map (homOfLE h).op)
      (ConcreteCategory.hom (S.g.val.app (op V)) t) :=
  (naturality_concrete S.g (homOfLE h) t).symm

-- Helper: restriction commutes with f application
private lemma res_f_eq {X : TopCat.{u}}
    {S : ShortComplex (TopCat.Sheaf AddCommGrpCat.{u} X)}
    {V W : Opens X} (h : W ≤ V)
    (a : S.X₁.val.obj (op V)) :
    ConcreteCategory.hom (S.X₂.val.map (homOfLE h).op)
      (ConcreteCategory.hom (S.f.val.app (op V)) a) =
    ConcreteCategory.hom (S.f.val.app (op W))
      (ConcreteCategory.hom (S.X₁.val.map (homOfLE h).op) a) :=
  naturality_concrete S.f (homOfLE h) a
