...
theorem exists_section_generating_stalks
  ...
  have mk_P {x : X} {hx : x ∈ V} {d : ℤ} (hpos : 0 < d) (hrange : ⟨d⟩ ∈ H_at x hx) (hgen : ∀ h ∈ H_at x hx, d ∣ h.down) : P d.toNat :=
  ...
  have hcoeff_x : i_x x (R.germ W x hxW sW) = d • gen_at x (hWV hxW) := by
    rw [Presheaf.germ_res_apply, Presheaf.stalkFunctor_map_germ_apply V₁ x (hWV₁ hxW) i s₁]
    simpa [map_zsmul, Presheaf.germ_res_apply] using congrArg ((Sheaf.zeroOutsideInt V).presheaf.germ W x hxW) hW_eq
  ...
  obtain ⟨d_x, hd_x_mem, hd_x_pos, hd_x_gen⟩ := H_at_cyclic <|
    AddSubgroup.ne_bot_iff_exists_ne_zero.mpr ⟨⟨⟨d⟩, ⟨_, hcoeff_x⟩⟩, fun h => ne_of_gt hd_pos (by simpa using congrArg ULift.down (congrArg Subtype.val h))⟩
  have hd_x_eq : d_x = d := by
    obtain ⟨k, hk⟩ := hd_x_gen ⟨d⟩ ⟨_, hcoeff_x⟩
    exact le_antisymm (Int.le_of_dvd hd_pos ⟨k, hk⟩) <| (Int.le_toNat (le_of_lt hd_x_pos)).mp (Nat.find_min' (p := P) hP (mk_P hd_x_pos hd_x_mem hd_x_gen))
  obtain ⟨k₀, rfl⟩ := hd_x_gen ⟨n⟩ ⟨a, hn⟩
  exact ⟨k₀, hi_inj x (by rw [map_zsmul, hn, hd_x_eq, mul_comm, mul_smul, ← hcoeff_x])⟩
