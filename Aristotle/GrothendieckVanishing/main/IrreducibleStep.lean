...
theorem exists_section_generating_stalks
  ...
  have H_at_cyclic : ∀ (x : X) (hx : x ∈ V), H_at x hx ≠ ⊥ →
    ...
    · rintro rfl
      exact hH <| eq_bot_iff.mpr fun y hy => by
    ...
    · have hh' := AddSubgroup.mem_map_of_mem AddEquiv.ulift.toAddMonoidHom hh
      ...
      obtain ⟨n, hn⟩ := hh'
      simpa [abs_dvd, dvd_def, mul_comm, ULift.ext_iff] using
        (abs_dvd a h.down).mpr ⟨n, by simpa [zsmul_eq_mul, mul_comm] using hn.symm⟩
  ...
  obtain ⟨W, hx₀W, iW1, _, hW_eq⟩ :=
    TopCat.Presheaf.germ_eq (TopCat.Sheaf.zeroOutsideInt V).val x₀ hx₀V₁ hx₀V₁ (i.app (op V₁) s₁) d_gen_res (by
