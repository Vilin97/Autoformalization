...
theorem exists_section_generating_stalks
  ...
  have H_at_cyclic : ∀ (x : X) (hx : x ∈ V), H_at x hx ≠ ⊥ →
    ...
    have ha_ne : a ≠ 0 := by
      ...
      rw [ha, heq, AddSubgroup.closure_singleton_zero, AddSubgroup.mem_bot] at hy'
      exact AddEquiv.ulift.injective (hy'.trans (map_zero _).symm)
    have ha_mem : (⟨a⟩ : ULift.{u} ℤ) ∈ H_at x hx := by
      have : a ∈ (H_at x hx).map AddEquiv.ulift.toAddMonoidHom :=
  ...
  have hP : ∃ n, P n := by
    ...
    obtain ⟨d', hd'_mem, hd'_pos, hd'_gen⟩ := H_at_cyclic x₀' hx₀'V hH'_ne
  ...
  · -- d_nat is minimal, so at every x ∈ W the generator d_x = d.down
    ...
    obtain ⟨d_x, hd_x_mem, hd_x_pos, hd_x_gen⟩ := H_at_cyclic x (hWV hxW) hHx_ne
