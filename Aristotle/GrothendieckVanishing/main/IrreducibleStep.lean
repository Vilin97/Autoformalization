...
theorem exists_section_generating_stalks
  ...
  have H_at_cyclic : ∀ (x : X) (hx : x ∈ V), H_at x hx ≠ ⊥ →
    ...
    · have hh' := AddSubgroup.mem_map_of_mem AddEquiv.ulift.toAddMonoidHom hh
      ...
      simpa [abs_dvd, dvd_def, mul_comm, ULift.ext_iff] using
        (show |a| ∣ AddEquiv.ulift.toAddMonoidHom h from (abs_dvd a _).mpr ⟨n, by simpa [zsmul_eq_mul, mul_comm] using hn.symm⟩)
  ...
  have hP : ∃ n, P n := by
    ...
    obtain ⟨d', hd'_mem, hd'_pos, hd'_gen⟩ := H_at_cyclic x₀' hx₀'V <|
      AddSubgroup.ne_bot_iff_exists_ne_zero.mpr ⟨⟨⟨n⟩, ⟨a₀', hn⟩⟩, fun h => by
        have h' : n = 0 := by simpa using congrArg ULift.down (congrArg Subtype.val h)
        exact ha₀' <| hi_inj x₀' <| by simpa [h'] using hn⟩
