...
theorem exists_section_generating_stalks
  ...
  have H_at_cyclic {x : X} {hx : x ∈ V} (hH : H_at x hx ≠ ⊥) : ∃ d : ℤ, ⟨d⟩ ∈ H_at x hx ∧ 0 < d ∧ ∀ h ∈ H_at x hx, d ∣ h.down := by
    obtain ⟨a, ha⟩ := Int.subgroup_cyclic ((H_at x hx).map AddEquiv.ulift.toAddMonoidHom)
    refine ⟨|a|, ?_, abs_pos.mpr ?_, fun h hh => ?_⟩
    · change AddEquiv.ulift.symm |a| ∈ H_at x hx
      rw [← AddSubgroup.mem_map_equiv, ha, AddSubgroup.mem_closure_singleton]
      exact ⟨Int.sign a, by simpa only [zsmul_eq_mul] using Int.sign_mul_self_eq_abs a⟩
    · exact fun ha0 => hH <| (AddSubgroup.map_eq_bot_iff_of_injective (H_at x hx) AddEquiv.ulift.injective).mp (by simpa [ha0, AddSubgroup.closure_singleton_zero] using ha)
    · change AddEquiv.ulift.symm h.down ∈ H_at x hx at hh
      rw [← AddSubgroup.mem_map_equiv, ha, AddSubgroup.mem_closure_singleton] at hh
      simpa using (abs_dvd a h.down).mpr (by simpa [dvd_def, zsmul_eq_mul, mul_comm, eq_comm] using hh)
  -- Step 1: P(n) holds if n > 0 and some x ∈ V has image subgroup generated by n
  let P : ℕ → Prop := fun n => 0 < n ∧ ∃ (x : X) (hx : x ∈ V),
    (⟨(n : ℤ)⟩ : ULift.{u} ℤ) ∈ H_at x hx ∧ ∀ h ∈ H_at x hx, (n : ℤ) ∣ h.down
  have mk_P : ∀ {x : X} {hx : x ∈ V} {d : ℤ}, 0 < d → ⟨d⟩ ∈ H_at x hx →
      (∀ h ∈ H_at x hx, d ∣ h.down) → P d.toNat := fun {x} {hx} {d} hpos hrange hgen =>
    ⟨Int.pos_iff_toNat_pos.mp hpos, x, hx, by simpa [Int.toNat_of_nonneg (le_of_lt hpos)] using hrange,
      fun h hh => by simpa [Int.toNat_of_nonneg (le_of_lt hpos)] using hgen h hh⟩
  have hP : ∃ n, P n := by
    ...
    obtain ⟨d', hd'_mem, hd'_pos, hd'_gen⟩ := H_at_cyclic <|
      AddSubgroup.ne_bot_iff_exists_ne_zero.mpr ⟨⟨⟨n⟩, ⟨a₀', hn⟩⟩, fun h => ha₀' <| hi_inj x₀' <| by
        simpa [show n = 0 by simpa using congrArg ULift.down (congrArg Subtype.val h)] using hn⟩
    exact ⟨_, mk_P hd'_pos hd'_mem hd'_gen⟩
  haveI : DecidablePred P := Classical.decPred P
  let d : ℤ := Nat.find (p := P) hP
  obtain ⟨hd_nat_pos, x₀, hx₀V, ⟨a₁, ha₁⟩, _⟩ := Nat.find_spec (p := P) hP
  ...
  obtain ⟨W, hx₀W, iW1, _, hW_eq⟩ :=
    TopCat.Presheaf.germ_eq (TopCat.Sheaf.zeroOutsideInt V).val x₀ hx₀V₁ hx₀V₁ (i.app (op V₁) s₁)
      (d • ConcreteCategory.hom ((TopCat.Sheaf.zeroOutsideInt V).val.map (homOfLE hV₁V).op) (TopCat.Sheaf.zeroOutsideInt.generator V)) (by
      rw [← TopCat.Presheaf.stalkFunctor_map_germ_apply V₁ x₀ hx₀V₁ i s₁,
  ...
  let sW := ConcreteCategory.hom (R.map (homOfLE hWV₁).op) s₁
  refine ⟨W, hWV, fun h => (Opens.mem_bot (x := x₀)).mp (h ▸ hx₀W), sW, fun x hxW => ?_⟩
  have hcoeff_x : i_x x (R.germ W x hxW sW) = d • gen_at x (hWV hxW) := by
    rw [TopCat.Presheaf.germ_res_apply, TopCat.Presheaf.stalkFunctor_map_germ_apply V₁ x (hWV₁ hxW) i s₁]
    simpa [map_zsmul, TopCat.Presheaf.germ_res_apply] using congrArg ((TopCat.Sheaf.zeroOutsideInt V).presheaf.germ W x hxW) hW_eq
  constructor
  ...
  · -- d is minimal, so at every x ∈ W the generator d_x = d
    intro a; obtain ⟨n, hn⟩ := stalk_zeroOutsideInt_eq_zsmul_generator V x (hWV hxW) (i_x x a)
    obtain ⟨d_x, hd_x_mem, hd_x_pos, hd_x_gen⟩ := H_at_cyclic <|
      AddSubgroup.ne_bot_iff_exists_ne_zero.mpr ⟨⟨⟨d⟩, ⟨_, hcoeff_x⟩⟩,
        fun h => ne_of_gt hd_pos (by simpa using congrArg ULift.down (congrArg Subtype.val h))⟩
    have hd_x_eq : d_x = d := by
      obtain ⟨k, hk⟩ := hd_x_gen ⟨d⟩ ⟨_, hcoeff_x⟩
      exact le_antisymm (Int.le_of_dvd hd_pos ⟨k, hk⟩)
        ((Int.le_toNat (le_of_lt hd_x_pos)).mp <|
          Nat.find_min' (p := P) hP (mk_P hd_x_pos hd_x_mem hd_x_gen))
    obtain ⟨k₀, hk₀⟩ := hd_x_gen ⟨n⟩ ⟨a, hn⟩
    exact ⟨k₀, hi_inj x (by rw [map_zsmul, hn, (by simpa [hd_x_eq, mul_comm] using hk₀ : n = k₀ * d), mul_smul, ← hcoeff_x])⟩
