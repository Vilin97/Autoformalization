...
theorem exists_section_generating_stalks
  ...
  have hi_inj (x : X) : Function.Injective (i_x x) :=
    Presheaf.stalkFunctor_map_injective_of_app_injective (f := i) (fun U => (ConcreteCategory.mono_iff_injective_of_preservesPullback _).mp inferInstance) x
  ...
  have cyclic_generator_of_coeff {x : X} {hx : x ∈ V} {d : ℤ} {a}
    ...
    exact ⟨e.natAbs, Int.natAbs_pos.mpr (fun he0 => hd (by simpa [he, he0, AddSubgroup.closure_singleton_zero] using (show d ∈ H_at x hx from ⟨a, ha⟩))), by simp [he],
      fun h hh => Int.natAbs_dvd.mpr (by simpa [he, AddSubgroup.mem_closure_singleton, dvd_def, zsmul_eq_mul, mul_comm, eq_comm] using hh)⟩
  ...
  obtain ⟨W, hx₀W, iW1, _, hW_eq⟩ :=
    Presheaf.germ_eq (Sheaf.zeroOutsideInt V).val x₀ (show x₀ ∈ V₁ from ⟨hx₀W₀, hx₀V⟩) (show x₀ ∈ V₁ from ⟨hx₀W₀, hx₀V⟩) (i.app (op V₁) s₁)
      ...
      rw [← Presheaf.stalkFunctor_map_germ_apply V₁ x₀ (show x₀ ∈ V₁ from ⟨hx₀W₀, hx₀V⟩) i s₁, (Presheaf.germ_res_apply R (homOfLE inf_le_left) x₀ (show x₀ ∈ V₁ from ⟨hx₀W₀, hx₀V⟩) s₀).trans hs₀]
  ...
  exact (hd_x_gen n ⟨a, hn⟩).imp fun k₀ hkn => hi_inj x (by rw [map_zsmul, hn, hkn, le_antisymm (Int.le_of_dvd hd_pos (hd_x_gen d ⟨_, hcoeff_x⟩)) <| Int.ofNat_le.mpr (Nat.find_min' hP ⟨hd_x_pos, x, hWV hxW, hd_x_mem, hd_x_gen⟩), mul_comm, mul_smul, ← hcoeff_x])
