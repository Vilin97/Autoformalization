...
theorem exists_section_generating_stalks
  ...
  have H_at_cyclic : ∀ (x : X) (hx : x ∈ V), H_at x hx ≠ ⊥ →
      ∃ d : ULift.{u} ℤ, d ∈ H_at x hx ∧ d.down > 0 ∧
        ∀ h ∈ H_at x hx, ∃ k : ℤ, h = ⟨k * d.down⟩ := fun x hx hH => by
    ...
    · rintro rfl
      exact hH <| (AddSubgroup.map_eq_bot_iff_of_injective (H_at x hx) AddEquiv.ulift.injective).mp (by simpa [AddSubgroup.closure_singleton_zero] using ha)
    · change AddEquiv.ulift.symm h.down ∈ H_at x hx at hh
      rw [← AddSubgroup.mem_map_equiv, ha, AddSubgroup.mem_closure_singleton] at hh
      simpa [dvd_def, mul_comm, ULift.ext_iff] using
        (abs_dvd a h.down).mpr (by simpa [dvd_def, zsmul_eq_mul, mul_comm, eq_comm] using hh)
  ...
  have mk_P : ∀ (x : X) (hx : x ∈ V) (d : ULift.{u} ℤ),
      ...
      (∀ h ∈ H_at x hx, ∃ k : ℤ, h = ⟨k * d.down⟩) → P d.down.toNat := fun x hx d hpos hrange hgen =>
    ⟨Int.pos_iff_toNat_pos.mp hpos, x, hx, by rwa [Int.toNat_of_nonneg (le_of_lt hpos)],
      fun m hm => by simpa [Int.toNat_of_nonneg (le_of_lt hpos), dvd_def, mul_comm, ULift.ext_iff] using (hgen ⟨m⟩ hm)⟩
  have hP : ∃ n, P n := by
    ...
    obtain ⟨d', hd'_mem, hd'_pos, hd'_gen⟩ := H_at_cyclic x₀' hx₀'V <|
  ...
  let d : ULift.{u} ℤ := ⟨(Nat.find (p := P) hP : ℤ)⟩
  obtain ⟨hd_nat_pos, x₀, hx₀V, ⟨a₁, ha₁⟩, _⟩ := Nat.find_spec (p := P) hP
  have hd_pos : d.down > 0 := Int.ofNat_lt.mpr hd_nat_pos
  ...
  obtain ⟨W, hx₀W, iW1, _, hW_eq⟩ :=
    TopCat.Presheaf.germ_eq (TopCat.Sheaf.zeroOutsideInt V).val x₀ hx₀V₁ hx₀V₁ (i.app (op V₁) s₁)
      (d.down • ConcreteCategory.hom ((TopCat.Sheaf.zeroOutsideInt V).val.map (homOfLE hV₁V).op) (TopCat.Sheaf.zeroOutsideInt.generator V)) (by
  ...
  refine ⟨W, hWV, fun h => (Opens.mem_bot (x := x₀)).mp (h ▸ hx₀W),
    ConcreteCategory.hom (R.map (homOfLE hWV₁).op) s₁, fun x hxW => ?_⟩
  have hcoeff_x :
      i_x x (R.germ W x hxW
        (ConcreteCategory.hom (R.map (homOfLE hWV₁).op) s₁)) =
      d.down • gen_at x (hWV hxW) := by
    rw [TopCat.Presheaf.germ_res_apply,
      TopCat.Presheaf.stalkFunctor_map_germ_apply V₁ x (hWV₁ hxW) i s₁]
    simpa [map_zsmul, TopCat.Presheaf.germ_res_apply] using
  ...
  · -- d_nat is minimal, so at every x ∈ W the generator d_x = d.down
    ...
    have hd_x_eq : d_x.down = d.down := by
      ...
      exact le_antisymm (Int.le_of_dvd hd_pos ⟨k, by simpa [mul_comm] using congrArg ULift.down hk⟩)
        ((Int.le_toNat (le_of_lt hd_x_pos)).mp <|
          Nat.find_min' (p := P) hP (mk_P x (hWV hxW) d_x hd_x_pos hd_x_mem hd_x_gen))
    obtain ⟨k₀, hk₀⟩ := hd_x_gen ⟨n⟩ ⟨a, hn⟩
    exact ⟨k₀, hi_inj x (by rw [map_zsmul, hn,
      (by simpa [hd_x_eq] using congrArg ULift.down hk₀ : n = k₀ * d.down), mul_smul, ← hcoeff_x])⟩
