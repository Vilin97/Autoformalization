...
theorem exists_section_generating_stalks
  ...
  have H_ne_of_coeff {x : X} {hx : x ∈ V} {d : ℤ} {a} (hd : d ≠ 0) (ha : i_x x a = d • gen_at x hx) : H_at x hx ≠ ⊥ :=
    AddSubgroup.ne_bot_iff_exists_ne_zero.mpr ⟨⟨d, ⟨a, ha⟩⟩, fun h => hd (by simpa using congrArg Subtype.val h)⟩
  have H_at_cyclic {x : X} {hx : x ∈ V} (hH : H_at x hx ≠ ⊥) :
      ∃ d : ℕ, 0 < d ∧ (d : ℤ) ∈ H_at x hx ∧ ∀ h ∈ H_at x hx, (d : ℤ) ∣ h := by
    obtain ⟨a, ha⟩ := Int.subgroup_cyclic (H_at x hx)
    refine ⟨a.natAbs, Int.natAbs_pos.mpr ?_, by simp [ha], fun h hh => ?_⟩
    · exact fun ha0 => hH (by simpa [ha0, AddSubgroup.closure_singleton_zero] using ha)
    · exact Int.natAbs_dvd.mpr (by simpa [ha, AddSubgroup.mem_closure_singleton, dvd_def, zsmul_eq_mul, mul_comm, eq_comm] using hh)
  ...
  have hP : ∃ n, P n := by
    ...
    obtain ⟨d', hd'_pos, hd'_mem, hd'_gen⟩ := H_at_cyclic <| H_ne_of_coeff (fun hn0 => ha₀' <| hi_inj x₀' <| by simpa [hn0] using hn) hn
    exact ⟨_, hd'_pos, x₀', hx₀'V, hd'_mem, hd'_gen⟩
  let d : ℤ := @Nat.find P (Classical.decPred P) hP
  obtain ⟨hd_nat_pos, x₀, hx₀V, ⟨a₁, ha₁⟩, _⟩ := @Nat.find_spec P (Classical.decPred P) hP
  ...
  obtain ⟨d_x, hd_x_pos, hd_x_mem, hd_x_gen⟩ := H_at_cyclic <| H_ne_of_coeff (ne_of_gt hd_pos) hcoeff_x
  have hd_x_eq : (d_x : ℤ) = d := le_antisymm (Int.le_of_dvd hd_pos (hd_x_gen d ⟨_, hcoeff_x⟩)) <|
    Int.ofNat_le.mpr (@Nat.find_min' P (Classical.decPred P) hP _ ⟨hd_x_pos, x, hWV hxW, hd_x_mem, hd_x_gen⟩)
