...
abbrev SectionIndex {C : Type*} [Category C] {FC : C → C → Type*} {CC : C → Type*}
    ...
    (F : Sheaf C X) :=
  Σ U : Opens X, ToType (F.presheaf.obj (op U))
...
abbrev finsetGeneratorMap {X : TopCat.{u}}
  ...
  familyMap
    (fun σ : {σ // σ ∈ S} => zeroOutsideInt σ.1.1)
    (fun σ => zeroOutsideInt.sHom (F := F) σ.1.2)
...
abbrev allSectionMap {X : TopCat.{u}}
  ...
  familyMap
    (fun σ : SectionIndex F => zeroOutsideInt σ.1)
    (fun σ => zeroOutsideInt.sHom (F := F) σ.2)
