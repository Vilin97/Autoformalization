...
theorem cohomology_vanishing_of_finitelyGenerated_vanishing
  ...
  have hHom : (∀ j, Subsingleton (Z ⟶ (finsetGenFunctor K).obj j)) →
    ...
    have hKsub : Subsingleton (K.val.obj (op ⊤)) := by
      -- Use CreatesColimit → PreservesColimit → section-level colimit → IsZero
      ...
      have hpre := isColimitOfPreserves (sheafToPresheaf _ _) (finsetGenCocone_isColimit K)
      have hsec := isColimitOfPreserves ((evaluation _ _).obj (op ⊤)) hpre
      have hzero : IsZero (finsetGenFunctor K ⋙
          sheafToPresheaf _ _ ⋙ (evaluation _ _).obj (op ⊤)) := by
        rw [Functor.isZero_iff]; intro j
        exact @AddCommGrpCat.isZero_of_subsingleton _ (hGsub j)
      have hzpt : IsZero (K.val.obj (op ⊤)) := hsec.isZero_pt hzero
      constructor; intro a b
      have := hzpt.eq_zero_of_tgt (𝟙 (K.val.obj (op ⊤)))
      have : ∀ (x : K.val.obj (op ⊤)), x = 0 := by
        intro x; have := AddCommGrpCat.ext_iff.mp this x; simp at this; exact this
      exact (this a).trans (this b).symm
