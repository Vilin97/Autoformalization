  ...
  have h_gauss : ∫ v : Fin 3 → ℝ,
    ...
    have h_gauss_integral_component :
      ...
      have h_fubini :
        ...
        simp +decide [ mul_assoc, ← Real.exp_sum,
          Finset.sum_add_distrib,
          Finset.mul_sum _ _ _, Finset.sum_mul, normSq ]
        simp +decide [ ← Real.exp_add, Fin.sum_univ_three, dotProduct ]
        congr
        ext
        ring!
      have h_fubini2 :
    ...
    simp +decide [ Real.exp_add, mul_add, add_comm,
  ...
  have h_const_deriv : ∃ c₀ : ℝ, ∀ v : Fin 3 → ℝ, ∀ w : Fin 3 → ℝ, (fderiv ℝ g v) w = c₀ • w := by
    ...
    have h_const_c : ∀ v w : Fin 3 → ℝ, c v = c w := by
      ...
      have h_second_deriv : ∀ v : Fin 3 → ℝ,
        ...
        exact fun i => DifferentiableAt.comp v
          (differentiableAt_pi.1
            (h_diff.contDiffAt.differentiableAt le_rfl) i)
          (differentiableAt_id)
      ...
      have h_const_c : ∀ v : Fin 3 → ℝ, (fderiv ℝ c v) = 0 := by
        ...
        ext w
        simp [h_zero_deriv]
        convert (show (fderiv ℝ c v) w =
            ∑ k : Fin 3, w k • (fderiv ℝ c v) (Pi.single k 1) from ?_)
          using 1
        · simp [h_zero_deriv]
