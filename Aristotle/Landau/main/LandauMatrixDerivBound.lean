...
-/
lemma eucNorm_le_sqrt_three_norm (z : Fin 3 → ℝ) : eucNorm z ≤ Real.sqrt 3 * ‖z‖ := by
...
-/
lemma normSq_le_three_norm_sq (z : Fin 3 → ℝ) : |normSq z| ≤ 3 * ‖z‖^2 := by
  unfold normSq;
  norm_num [ dotProduct ];
...
-/
lemma normSq_hasFDerivAt (z : Fin 3 → ℝ) :
    HasFDerivAt normSq (dNormSq z) z := by
      ...
      have h_bound : ∀ x : Fin 3 → ℝ, ‖normSq (z + x) - normSq z - (dNormSq z) x‖ ≤ 3 * ‖x‖^2 := by
        intro x; exact (by
        convert normSq_le_three_norm_sq x using 1 ; norm_num [ normSq, dNormSq ] ; ring;
...
-/
lemma eucNorm_hasFDerivAt {z : Fin 3 → ℝ} (hz : z ≠ 0) :
    HasFDerivAt eucNorm (dEucNorm z) z := by
      convert HasFDerivAt.sqrt ( normSq_hasFDerivAt z ) _ using 1;
      · ext; norm_num [ dEucNorm, dNormSq ] ; ring;
        rfl;
      · exact ne_of_gt ( normSq_pos hz )
...
lemma innerLandauEntry_hasFDerivAt (z : Fin 3 → ℝ) (i j : Fin 3) :
    HasFDerivAt (fun z => innerLandauMatrix z i j) (dInnerLandauEntry z i j) z := by
      ...
      convert HasFDerivAt.sub ( HasFDerivAt.smul ( normSq_hasFDerivAt z ) ( hasFDerivAt_const _ _ ) ) ( crossTerm_hasFDerivAt z i j ) using 1 ; norm_num [ Matrix.vecMulVec ] ; ring!;
...
`|z · v| ≤ ‖z‖_2 * ‖v‖_2`.
-/
lemma abs_dotProduct_le_eucNorm_mul_eucNorm (z v : Fin 3 → ℝ) :
...
-/
lemma eucNorm_pos {z : Fin 3 → ℝ} (hz : z ≠ 0) : 0 < eucNorm z := by
  unfold eucNorm
  exact Real.sqrt_pos.mpr (normSq_pos hz)
    ...
    ‖dPsiNorm Ψ z‖ ≤ Real.sqrt 3 * |deriv Ψ (eucNorm z)| := by
      ...
      have h_bound : ∀ x : Fin 3 → ℝ, |dotProduct z x| ≤ eucNorm z * Real.sqrt 3 * ‖x‖ := by
        ...
        have h_cauchy_schwarz : |dotProduct z x| ≤ eucNorm z * eucNorm x := by
          exact abs_dotProduct_le_eucNorm_mul_eucNorm z x
        have h_eucNorm_le_sqrt_three_norm : eucNorm x ≤ Real.sqrt 3 * ‖x‖ := by
          exact eucNorm_le_sqrt_three_norm x
        exact le_trans h_cauchy_schwarz (by
        simpa only [ mul_assoc ] using mul_le_mul_of_nonneg_left h_eucNorm_le_sqrt_three_norm ( eucNorm_nonneg z ));
      intro x; rw [ abs_of_nonneg ( eucNorm_nonneg z ) ] ; convert mul_le_mul_of_nonneg_right ( mul_le_mul_of_nonneg_left ( h_bound x ) ( abs_nonneg ( deriv Ψ ( eucNorm z ) ) ) ) ( inv_nonneg.mpr ( eucNorm_nonneg z ) ) using 1 ; ring;
      rw [ mul_inv_cancel_right₀ ( ne_of_gt ( eucNorm_pos hz ) ) ]
...
lemma landauEntry_hasFDerivAt_of_ne_zero
    ...
    HasFDerivAt (fun z => landauMatrix Ψ z i j) (dLandauEntry Ψ z i j) z := by
      ...
      have h_prod_rule : HasFDerivAt (fun z => Ψ (eucNorm z)) (dPsiNorm Ψ z) z ∧ HasFDerivAt (fun z => innerLandauMatrix z i j) (dInnerLandauEntry z i j) z := by
        ...
        convert HasFDerivAt.comp z ( hΨ_diff.differentiable le_rfl |> Differentiable.differentiableAt |> DifferentiableAt.hasFDerivAt ) ( eucNorm_hasFDerivAt hz ) using 1;
...
-- Aristotle-generated proof needs extra heartbeats after primed→VML def migration
