#!/usr/bin/env python3
"""Compute and emit class-representative trace certificates for row 23.

No previously certified ordinary row has dimension ten, so a representation
intertwiner cannot transport row 23 from the completed table fragment.  The
generated Lean modules instead recheck the five matrix multiplications used
by the two long class words directly.
...
def main() -> None:
    ...
    if arguments.write_lean:
        helper.write_long_word_certificates(23, matrix_a, matrix_b)
        output_directory = ROOT / "McKayConjecture" / "InductiveMcKay"
        for path in output_directory.glob(
            "AlternatingSixAmbientRow23LongWord*.lean"
        ):
            source = path.read_text(encoding="utf-8")
            path.write_text(
                source.replace(
                    "scripts/a6_row19_representative_traces.py",
                    "scripts/a6_row23_representative_traces.py",
                ).replace(
                    ...
                    # Both integral long-word trace goals close during
                    # simplification, so the generic empty-quotient
                    # fallback would run after the goal has disappeared.
                    "  ]; ring\n",
                    "  ]\n",
                ),
                encoding="utf-8",
    ...
    # The row-23 scalar pattern is trivial.  Thus a^-1 = a and b^-1 = b^3.
    matrix_a_inverse = matrix_a
