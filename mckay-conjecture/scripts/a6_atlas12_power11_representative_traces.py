#!/usr/bin/env python3
"""Compute and emit representative traces for ATLAS degree 12, power 11.
...
DUMMY_ROW = 1211
DUMMY_MODULE_STEM = "AlternatingSixAmbientRow1211"
TARGET_MODULE_STEM = "AlternatingSixAmbientAtlas12P11"
DUMMY_LEAN_STEM = "alternatingSixAmbientRow1211"
TARGET_LEAN_STEM = "alternatingSixAmbientAtlas12P11"
DUMMY_MATRIX_STEM = "alternatingSixFiveAmbient_row1211"
TARGET_MATRIX_STEM = "alternatingSixFiveAmbient_atlas12Power11"
LONG_WORD_SUFFIXES = (
    "LongWordData",
    "LongWordProductAB",
    "LongWordProductABB",
    ...
    for suffix in LONG_WORD_SUFFIXES:
        ...
        rewritten = (
            source.replace(
                ...
                "scripts/a6_atlas12_power11_representative_traces.py",
            )
            ...
            .replace("row-1211", "ATLAS degree-12 power-eleven")
            .replace(
...
def main() -> None:
    ...
    # Here a^2 = zeta^100 and b^4 = zeta^60. Thus
    # a^-1 = zeta^20 a and b^-1 = zeta^60 b^3.
    matrix_a_inverse = helper.scalar_multiply(
        base.Polynomial.monomial(20), matrix_a
    )
    ...
    kappa = base.Polynomial.monomial(100).reduce()
    ab_inverse_bridge_quotient = helper.reduction_quotient(
        kappa ** 4 * values["ab"],
        expected_ab_inverse,
    )
    abb_inverse_bridge_quotient = helper.reduction_quotient(
        kappa ** 2 * values["abb"],
        expected_abb_inverse,
    )
    print(f"kappa_reduced = {base.polynomial_to_lean(kappa)}")
    central_pairing_quotient = helper.reduction_quotient(
        kappa ** 4 * values["ab"] ** 2
        + kappa ** 2 * values["abb"] ** 2,
        base.Polynomial.constant(3),
    )
    print(
    ...
    print(
        "ab_inverse_bridge_quotient = "
        f"{base.polynomial_expression_to_lean(ab_inverse_bridge_quotient)}"
    )
    print(
        "abb_inverse_bridge_quotient = "
        f"{base.polynomial_expression_to_lean(abb_inverse_bridge_quotient)}"
    )
    print(
