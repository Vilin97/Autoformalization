#!/usr/bin/env python3
"""Compute representative traces for ATLAS degree 12, power 7.

No proved intertwiner transports another degree-twelve row to the displayed
power-seven matrices. The generated Lean modules therefore check the five
matrix multiplications used by the two long class words directly, with one
expensive multiplication per file under the stock limits.
...
DUMMY_ROW = 1207
DUMMY_MODULE_STEM = "AlternatingSixAmbientRow1207"
TARGET_MODULE_STEM = "AlternatingSixAmbientAtlasTwelvePowerSeven"
DUMMY_LEAN_STEM = "alternatingSixAmbientRow1207"
TARGET_LEAN_STEM = "alternatingSixAmbientAtlasTwelvePowerSeven"
DUMMY_MATRIX_STEM = "alternatingSixFiveAmbient_row1207"
TARGET_MATRIX_STEM = "alternatingSixFiveAmbient_atlas12Power7"
LONG_WORD_SUFFIXES = (
    "LongWordData",
    "LongWordProductAB",
    "LongWordProductABB",
    ...
    for suffix in LONG_WORD_SUFFIXES:
        ...
        rewritten = (
            source.replace(
                ...
                "scripts/a6_atlas12_power7_representative_traces.py",
            )
            ...
            .replace("row-1207", "ATLAS degree-12 power-seven")
            .replace(
        ...
        rewritten = "\n".join(
            ...
            if line.startswith("import ") and len(line) > 100
            else line
...
def main() -> None:
    ...
    expected_ab_inverse = polynomial(
        [(-1, 0), (-1, 4), (1, 16), (1, 20)]
    )
    expected_abb = polynomial([(1, 4), (-1, 16)])
    ...
    print(f"ab_expected_match = {values['ab'] == expected_ab}")
    print(
        "ab_inverse_expected_match = "
        f"{inverse_values['ab'] == expected_ab_inverse}"
    )
    print(f"abb_expected_match = {values['abb'] == expected_abb}")
    print(
    ...
    combined_pairing_quotient = helper.reduction_quotient(
        expected_ab * expected_ab_inverse
        + expected_abb * expected_abb_inverse,
        base.Polynomial.constant(3),
    )
    eta = base.Polynomial.monomial(20)
    central_pairing_quotient = helper.reduction_quotient(
        eta ** 4 * expected_ab ** 2
        + eta ** 2 * expected_abb ** 2,
        base.Polynomial.constant(3),
    )
    ab_inverse_quotient = helper.reduction_quotient(
        expected_ab_inverse,
        eta ** 4 * expected_ab,
    )
    abb_inverse_quotient = helper.reduction_quotient(
        expected_abb_inverse,
        eta ** 2 * expected_abb,
    )
    print(
        "ab_inverse_eq_eta_four_mul_ab = "
        f"{expected_ab_inverse == (eta ** 4 * expected_ab).reduce()}"
    )
    print(
        "abb_inverse_eq_eta_two_mul_abb = "
        f"{expected_abb_inverse == (eta ** 2 * expected_abb).reduce()}"
    )
    print(
    ...
    print(
        "ab_inverse_quotient = "
        f"{base.polynomial_expression_to_lean(ab_inverse_quotient)}"
    )
    print(
        "abb_inverse_quotient = "
        f"{base.polynomial_expression_to_lean(abb_inverse_quotient)}"
    )
