...
\begin{itemize}
  ...
  \item a modern equivariant version still assumes a \(p\)-solvable
  quotient \cite{MaltempoVallejo2026};
...
\emph{numerical} formalization.  In the proof of
\cite[Theorem 2.4]{MaltempoVallejo2026}, Equation (2.1) compares, one
Gallagher linear parameter at a time, the numbers of automorphism-fixed
characters above a canonical extension and above the corresponding
Glauberman extension.  Applied to the normal \(p'\)-kernel in Rossi's
central-intersection branch, this gives the ordinary-character count
underlying each matched Clifford inertia fibre.  The common quotient
parameter in the paper must be transported in Lean through the canonical
source--target quotient equivalence; it is not a definitionally identical
term on the two sides.  Likewise, the ambient normal \(p'\)-kernel must be
identified with its internal copy in the subgroup used by the existing
Theorem~4.4 API before asserting a literal character equality.  The automatic
\(p'\)-degree theorem \cite[Theorem~5.12]{Navarro2018} used in the proof
of Theorem~2.4 must then be applied explicitly before passing to the
project's \(p'\)-degree fibre types.
After adjoining the ambient center,
arbitrary equivalences between those finite fibres automatically preserve
the central scalar needed by the numerical reduction.  Thus equal finite
cardinalities can be converted to equivalences without formalizing the
Dade-algebra/projective-obstruction construction.

This shortcut does not replace the classification or the quasisimple
verification: the Maltempo--Vallejo argument still requires the relevant
quotient to be \(p\)-solvable.  The efficient formal route is therefore
hybrid: retain the CFSG/\(\IMK\) layer reduction, but use the
Okuyama--Wajima counting argument for the solvable normal-subgroup seam.
