...
/-!
# Explicit fourth symmetric powers of two-dimensional representations

For a two-by-two matrix, this file gives the matrix of its action on
homogeneous quartic polynomials in the ordered basis

`x⁴, x³y, x²y², xy³, y⁴`.

The construction is multiplicative, hence sends an arbitrary
two-dimensional matrix representation to a five-dimensional
...
namespace SymmetricFourthPower

variable {R : Type*} [CommRing R]

/-- Matrix of the fourth-symmetric-power action, in the basis
`x⁴, x³y, x²y², xy³, y⁴`.  Columns are the images of basis vectors. -/
def matrix (M : Matrix (Fin 2) (Fin 2) R) :
  ...
  !![
    ...
    4 * a ^ 3 * c, a ^ 3 * d + 3 * a ^ 2 * b * c,
      2 * a ^ 2 * b * d + 2 * a * b ^ 2 * c,
      3 * a * b ^ 2 * d + b ^ 3 * c, 4 * b ^ 3 * d;
    6 * a ^ 2 * c ^ 2,
      3 * a ^ 2 * c * d + 3 * a * b * c ^ 2,
      a ^ 2 * d ^ 2 + 4 * a * b * c * d + b ^ 2 * c ^ 2,
      3 * a * b * d ^ 2 + 3 * b ^ 2 * c * d,
      6 * b ^ 2 * d ^ 2;
    4 * a * c ^ 3, 3 * a * c ^ 2 * d + b * c ^ 3,
      2 * a * c * d ^ 2 + 2 * b * c ^ 2 * d,
      a * d ^ 3 + 3 * b * c * d ^ 2, 4 * b * d ^ 3;
...
/-- Scalars act on the fourth symmetric power by their fourth power. -/
theorem matrix_scalar (r : R) :
    matrix (Matrix.scalar (Fin 2) r) =
      Matrix.scalar (Fin 5) (r ^ 4) := by
  ext i j
...
private theorem matrix_mul_row_zero
    (M N : Matrix (Fin 2) (Fin 2) R) (j : Fin 5) :
...
/-- Fourth symmetric power as a multiplicative map on matrix monoids. -/
def matrixMonoidHom :
    Matrix (Fin 2) (Fin 2) R →*
      Matrix (Fin 5) (Fin 5) R where
  toFun := matrix
  map_one' := matrix_one
  map_mul' := matrix_mul
...
/-- Fourth symmetric power of an invertible two-by-two matrix. -/
def generalLinearGroupHom :
    Matrix.GeneralLinearGroup (Fin 2) R →*
      Matrix.GeneralLinearGroup (Fin 5) R :=
...
/-- The trace of a fourth symmetric power depends only on the original
trace and determinant. -/
theorem trace_matrix
    (M : Matrix (Fin 2) (Fin 2) R) :
    Matrix.trace (matrix M) =
      Matrix.trace M ^ 4 -
        3 * Matrix.det M * Matrix.trace M ^ 2 +
          Matrix.det M ^ 2 := by
  simp [matrix, Matrix.trace, Matrix.det_fin_two,
    Fin.sum_univ_succ]
  ring

/-- For determinant-one matrices, the fourth-symmetric-power trace is
`t⁴ - 3t² + 1`. -/
theorem trace_matrix_of_det_one
    (M : Matrix (Fin 2) (Fin 2) R)
    (hdet : Matrix.det M = 1) :
    Matrix.trace (matrix M) =
      Matrix.trace M ^ 4 - 3 * Matrix.trace M ^ 2 + 1 := by
...
/-- Fourth symmetric power of a two-dimensional matrix representation. -/
def representation
    (ρ : G →*
      Matrix.GeneralLinearGroup (Fin 2) ℂ) :
    Representation ℂ G (Fin 5 → ℂ) :=
  (Units.coeHom
      ((Fin 5 → ℂ) →ₗ[ℂ] (Fin 5 → ℂ))).comp
    ((Matrix.GeneralLinearGroup.toLin :
        Matrix.GeneralLinearGroup (Fin 5) ℂ ≃*
          LinearMap.GeneralLinearGroup ℂ (Fin 5 → ℂ))
      |>.toMonoidHom.comp
...
/-- The finite-dimensional object attached to a fourth-symmetric-power
representation. -/
abbrev fdRep
    (ρ : G →*
      Matrix.GeneralLinearGroup (Fin 2) ℂ) :
...
theorem representation_character
    ...
    (representation ρ).character g =
      Matrix.trace
          (ρ g : Matrix (Fin 2) (Fin 2) ℂ) ^ 4 -
        3 * Matrix.det
          (ρ g : Matrix (Fin 2) (Fin 2) ℂ) *
            Matrix.trace
              (ρ g : Matrix (Fin 2) (Fin 2) ℂ) ^ 2 +
        Matrix.det
          (ρ g : Matrix (Fin 2) (Fin 2) ℂ) ^ 2 := by
  rw [show
      (representation ρ).character g =
        LinearMap.trace ℂ (Fin 5 → ℂ)
          (Matrix.toLin'
            (matrix
              (ρ g :
...
/-- Character formula for a fourth symmetric power when the original
