...
LOG_FILE="$LOG_DIR/aristotle-codex-prover.log"
LOCK_FILE="$LOG_DIR/aristotle-codex-prover.lock"
...
if ! flock -n 9; then
  printf '[%s] skip: previous codex prover run is still active\n' "$(date -Is)" >> "$LOG_FILE"
  exit 0
fi

{
  printf '\n[%s] starting codex prover run\n' "$(date -Is)"
  cd "$REPO_DIR"
  status=0
  /home/vilin/.nvm/versions/node/v22.22.1/bin/codex exec \
    --cd "$REPO_DIR" \
    --color never \
    --ephemeral \
    --dangerously-bypass-approvals-and-sandbox \
    "$PROMPT" || status=$?
  printf '[%s] codex exit status: %s\n' "$(date -Is)" "$status"
  exit "$status"
} >> "$LOG_FILE" 2>&1
