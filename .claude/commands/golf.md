...
Run the automated finder first:
```bash
python3 scripts/find_repeated_blocks.py <project>/main/ --min-lines 4 --top 15
```
This finds repeated consecutive tactic blocks across all files, ranked by
potential savings (lines × occurrences). Focus on blocks that are actual
proof logic (not just repeated signatures or `have` type annotations).

For manual search, grep for patterns like:
