...
Read `critique.md`. Extract every open (non-strikethrough) issue. Discard issues that belong to `/prove` or `/simplify`. Everything remaining is a `/strengthen` target.

Also read `plan.md` — it may contain multi-cycle strategies or experimental results from previous cycles that inform what to attempt next.
...
**Do not default to the easiest item.** Weigh impact against effort honestly. An ambitious change that requires experimentation is fine — use the project's `experiments/` directory to prototype ideas without touching `main/`. A failed experiment that rules out an approach or reveals a subtlety is still progress worth documenting.

## Phase 3: Execute

- For changes ready for `main/`: edit files in the project's `main/` directory, build to verify.
- For exploratory work: create files in the project's `experiments/` directory. Document what you tried and what you learned.
