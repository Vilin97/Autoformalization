...
The two compact cut sides have now also been reglued directly.  Their common attaching maps are
the same labelled zero section of the full oriented compact collar; their carrier intersection is
exactly that seam and their carrier union is the whole compact coordinate exterior.  These facts
first give a point-set homeomorphism from the corresponding topological pushout to
`CoordinateUnlinkExteriorEighthPowerPullback m`, equal to subtype inclusion on both pieces.  The
pushout is then equipped with the atlas generated by the two direct cut interiors and the full
common-model signed collar.  The two off-seam composites are the native ambient open-side maps,
and the seam composite is the full oriented collar after an explicit common-to-natural source
rechart.  Three-generator recognition upgrades the same point-set comparison to

```text
CoordinateUnlinkExteriorPlanarCompactCutPushout m
  diffeomorphic to CoordinateUnlinkExteriorEighthPowerPullback m.
```
